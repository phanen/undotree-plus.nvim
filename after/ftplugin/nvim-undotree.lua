...
local destroy = vim.F.nil_wrap(function()
  api.nvim_win_close(require('undotree-plus').diff_win, true)
...
render_diff = vim.schedule_wrap(function()
  ...
  local buf = require('undotree-plus').buf_from_title(api.nvim_buf_get_name(ut_buf))
  ...
  require('undotree-plus').render_diff(buf, require('undotree-plus._').asinteger(line:match('%d+')))
end)
...
api.nvim_create_autocmd('BufEnter', {
  ...
  callback = vim.schedule_wrap(function(ev)
    ...
    require('undotree-plus').open({ buf = ut_buf, win = vim.b[buf].nvim_undotree })
